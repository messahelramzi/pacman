...
// ETI translation units (MLS_eti_*.cpp).  Extern template declarations in
...
py::array_t<fp_t> RunInterpolate(np_array<coordinates_t> &rSourcePoints,
  ...
  Transfer<ExecSpace, Dim> transfer(TransferMethods::MLS);
  ...
  auto MLS_interpolator =
      MLS::MLSInterpolator<ExecSpace, Dim>(transfer);
...
/// @brief Dispatch runtime dimension to a compile-time template argument.
template <int_t Dim>
  ...
  return std::visit(
      [&](auto execSpaceObj) {
        return RunInterpolate<decltype(execSpaceObj), Dim>(
            rSourcePoints, rSourceValues, rTargetPoints);
      },
...
py::array_t<fp_t> Interpolate(const int_t spaceDimension,
  ...
  const std::string _region_name = "PyBindingsMLS::Interpolate";
