...
/**
 * @file FETools.hpp
 * @brief Finite-element geometric helpers and Lagrange-space specializations.
 *
 ...
 * - shape-function values and derivatives used by FE interpolation/Newton
 * routines.
 */
...
/**
 * @brief Generic predicate to test if local coordinates lie inside a reference
 * geometry.
 * @tparam T Geometric support kind (`POINT`, `LINE`, `TRIANGLE`, ...).
 * @param[in] xi First local coordinate.
 * @param[in] eta Second local coordinate.
 * @param[in] phi Third local coordinate.
 * @return `true` when `(xi, eta, phi)` is inside the reference support.
 * @note The primary template is intentionally not implemented and aborts at
 * runtime. Supported behavior is provided through explicit specializations.
 */
...
/**
 * @brief Compile-time FE geometry interface for one cell type.
 * @tparam ExecSpace Kokkos execution space used by temporary views.
 * @tparam CT PACMAN/VTK cell type.
 *
