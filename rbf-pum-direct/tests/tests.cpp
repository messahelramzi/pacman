...
template <typename ExecSpace, typename RbfFunc, int Dim>
                          ...
                          const int iterations)
{
    ...
    size_t N, M;
    auto source_grid =
        get_points_from_vtu_grid<Dim, RbfPumFPType>(src.c_str(), &N);
    auto target_grid =
        get_points_from_vtu_grid<Dim, RbfPumFPType>(dst.c_str(), &M);

    using PointsView = Kokkos::View<ArborX::Point<Dim, RbfPumFPType>*,
                                    Kokkos::DefaultHostExecutionSpace>;
    using ScalarsView =
        Kokkos::View<RbfPumFPType*, Kokkos::DefaultHostExecutionSpace>;
    ...
    Kokkos::parallel_for(
        "test::fill source and values", Kokkos::RangePolicy(hostspace, 0, N),
        [&](const int& i) {
            source(i) = source_grid[i];
            values(i) = franke_function<RbfPumFPType>(source_grid[i]);
        });
    Kokkos::parallel_for(
        "test::fill target", Kokkos::RangePolicy(hostspace, 0, M),
        [&](const int& i) {
            target(i) = target_grid[i];
            reference(i) = franke_function<RbfPumFPType>(target_grid[i]);
        });
    Kokkos::fence();
    ...
    auto interpolator = RbfPumInterpolator<ExecSpace, Dim, RbfPumFPType,
                                           decltype(rbf_function)>(
    ...
    for (int it = 0; it < iterations; ++it)
    {
        const auto begin =
            std::chrono::high_resolution_clock::now().time_since_epoch();
        auto interpolator2 = RbfPumInterpolator<ExecSpace, Dim, RbfPumFPType,
                                                decltype(rbf_function)>(
            source, values, target, rbf_function, 50, 0.15, 0.1);
        const auto end =
            std::chrono::high_resolution_clock::now().time_since_epoch();
    ...
    Kokkos::sort(errors);
    RbfPumFPType sum = 0.0;
    Kokkos::parallel_reduce(
        "sum", Kokkos::RangePolicy(hostspace, 0, M),
        [&](const int i, RbfPumFPType& update) { update += errors(i); }, sum);
    RbfPumFPType mean = sum / M;

    RbfPumFPType min_val, max_val;
    ...
    auto worst_idx = [&](double pct) {
        size_t i = static_cast<size_t>(pct * M);
        if (i >= M)
            i = M - 1;
        return i;
    };

    size_t i1 = worst_idx(0.99);
    size_t i5 = worst_idx(0.95);
    size_t i10 = worst_idx(0.90);
    size_t i50 = worst_idx(0.50);

    RbfPumFPType worst1, worst5, worst10, worst50;
...
template <int Dim>
int dispatch(const std::string& execspace, int iterations,
             ...
             const std::string& target_mesh)
{
    IterationResult res = std::visit(
        [&](auto execspace_t, auto rbf_function_t) {
            return _run_test<decltype(execspace_t), decltype(rbf_function_t),
                             Dim>(source_mesh, target_mesh, iterations);
        },
        make_execspace(execspace), make_rbffunction<RbfPumFPType>(func));
...
int main(int argc, char* argv[])
{
    ...
    Kokkos::ScopeGuard guard;
    {
        ...
        {
        case 1:
            return dispatch<1>(str_execspace, iterations, str_function,
                               str_source, str_target);
        case 2:
            return dispatch<2>(str_execspace, iterations, str_function,
                               str_source, str_target);
        case 3:
            return dispatch<3>(str_execspace, iterations, str_function,
                               str_source, str_target);
        default:
