...
Note: some older versions of `cmake` and `cuda` may work but have not been tested.

## Configure the CMake project

```shell
cmake -DUSE_DEVICE=[ON/OFF] -DCMAKE_BUILD_TYPE=Release -S . -B build
```
...
out = RbfPumInterpolator.interpolate(..., ..., ...)
```
